...
Iotas.pattern = {
    new = function(self, direction, angles)
        data = {
...
Iotas.FrameEvaluate = {
    new = function(self, hex)
        data = {
            Type = "hexcasting:FrameEvaluate",
            Hex = hex
        }
        self.__index = self
        return setmetatable(data,self)
    ...
    eval = function(self, castEnv)
        local iota = table.remove(self.Hex.list, 1)
        if iota then
            iota:eval(castEnv)
        else
...
Iotas.FrameForEach = {
    new = function(self, data, hex, stack)
        data = {
            Type = "hexcasting:FrameForEach",
            Hex = hex,
            Data = data,
            originalStack = stack,
            outputs = Hexcasting.Iotas.hexcasting.list:new()
        }
        self.__index = self
        return setmetatable(data,self)
    end,
    eval = function(self, castEnv)
        for _, iota in pairs(castEnv.stack.list) do
            self.outputs:append(iota)
        end
        castEnv.stack = self.originalStack:copy()
